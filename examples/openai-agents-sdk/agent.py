...
==================================================
Demonstrates jurisdiction-aware compliance guardrails for AI agents operating
in African financial markets, implemented with the OpenAI Agents SDK using only
stdlib Python (no external compliance engine required).

Four jurisdictions are covered:
  ...
  Kenya        Data Protection Act 2019 s.49
               Cross-border personal data transfer blocked without documented consent
               (Enforcing authority: Office of the Data Protection Commissioner)

  Ghana        Data Protection Act 2012 (Act 843) s.18(2)
               Cross-border transfer requires adequacy finding or consent;
...
Policy source: https://github.com/kingztech2019/agt-policies-nigeria
...
# "account balance: ₦15M" is not blocked. Negative lookahead on 'transfer'
# excludes noun phrases ("transfer limit", "transfer fee") where transfer is a
# modifier, not an action verb. 'request' is intentionally NOT excluded so that
# "submit a transfer request for ₦15M" is correctly blocked.
...
# [^.!?] prevents cross-sentence contamination from a financial-transfer verb in
# sentence 1 reaching a data-object keyword in sentence 2.
...
def _za_id_date_valid(id_str: str) -> bool:
    """Return True only if the YYMMDD prefix is a real calendar date in either century."""
    ...
    if isinstance(input, str):
        ...
        for item in target:
            ...
            if isinstance(content, str):
                ...
                # SDK uses "input_text" for user message parts, "text" for assistant parts.
                for part in content:
...
async def main() -> None:
    scenarios = [
        ...
        (
            "Kenya — KDPA s.49 cross-border data transfer without consent",
    ...
    for label, user_input, ctx in scenarios:
